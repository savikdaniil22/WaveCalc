...
const qualityStyles = {
  ...
  'Пограничный канал': 'bg-amber-500/20 text-amber-200',
...
export const ResultsDashboard = () => {
  ...
  const chartData = useMemo(
    () =>
      results
        ? [
            { name: 'Мощность TX', value: params.txPowerDbm },
            { name: 'Суммарные потери', value: -results.totalLossDb },
            { name: 'Мощность RX', value: results.receivedPowerDbm },
            { name: 'Чувствительность', value: params.rxSensitivityDbm },
            { name: 'Запас', value: results.fadeMarginDb },
          ]
        : [],
    ...
    <div className="grid gap-4 md:grid-cols-2">
      <Card className="md:col-span-2">
        <CardTitle className="flex items-center justify-between">
          <span>Модуль 3 - Панель качества радиолинии</span>
        ...
        <CardContent className="grid gap-3 md:grid-cols-3">
          <Metric title="FSPL" value={`${results.fsplDb.toFixed(2)} dB`} />
          <Metric title="Мощность на входе RX" value={`${results.receivedPowerDbm.toFixed(2)} dBm`} />
          ...
          <Metric title="dBm -> mW" value={results.dbmToMw.toFixed(4)} />
          <Metric title="dBm -> мкВ (50 Ом)" value={results.dbmToUv50.toFixed(2)} />
        </CardContent>
      </Card>

      <Card>
        <CardTitle>График энергетического баланса (Recharts)</CardTitle>
      ...
      <Card>
        <CardTitle>Визуализация зоны Френеля (SVG)</CardTitle>
        <CardContent className="h-72">
          <FresnelVisualizer
